...
if(isset($_GET['q']) && $_GET['q'] == "edit" ){
	if(!empty($_GET['id'])) {
		$select = $wpdb->get_results( "SELECT * FROM `{$wpdb->prefix}civi_member_sync`WHERE `id` =".$_GET['id']);
...
if ($_POST) {
	...
	if(empty($sameType) && empty($errors)) {
		$insert = $wpdb->get_results( "REPLACE INTO  {$wpdb->prefix}civi_member_sync SET `wp_role`='$wp_role', `civi_mem_type`=$civi_member_type, `current_rule`='$current_rule',`expiry_rule`='$expiry_rule', `expire_wp_role`='$expired_wp_role'" );
